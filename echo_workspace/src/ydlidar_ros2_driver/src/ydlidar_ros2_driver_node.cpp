...
int main(int argc, char *argv[]) {
  ...
  ///ignore array
  str_optvalue = "";
  node->declare_parameter("ignore_array", std::string(""));
  node->get_parameter("ignore_array", str_optvalue);
  laser.setlidaropt(LidarPropIgnoreArray, str_optvalue.c_str(), str_optvalue.size());

  std::string frame_id = "laser_frame";
  node->declare_parameter("frame_id", std::string("laser_frame"));
  node->get_parameter("frame_id", frame_id);

  //////////////////////int property/////////////////
  /// lidar baudrate
  int optval = 230400;
  node->declare_parameter("baudrate", 128000);
  ...
  node->declare_parameter("device_type", 6);
  node->get_parameter("device_type", optval);
  laser.setlidaropt(LidarPropDeviceType, &optval, sizeof(int));
  /// sample rate
  optval = 9;
  node->declare_parameter("sample_rate", 5);
  ...
  /// Intenstiy bit count
  optval = 0;
  node->declare_parameter("intensity_bit", 8);
  ...
  /// rotate 180
  b_optvalue = true;
  node->declare_parameter("reversion", false);
  node->get_parameter("reversion", b_optvalue);
  laser.setlidaropt(LidarPropReversion, &b_optvalue, sizeof(bool));
  /// Counterclockwise
  b_optvalue = true;
  node->declare_parameter("inverted", false);
  ...
  /// unit: m
  f_optvalue = 64.f;
  node->declare_parameter("range_max", 16.0);
